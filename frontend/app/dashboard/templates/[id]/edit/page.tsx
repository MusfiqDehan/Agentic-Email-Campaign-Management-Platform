...
export default function EditTemplatePage() {
  ...
  const { register, handleSubmit, setValue, watch, control, reset, formState: { errors } } = useForm<TemplateFormValues>({
  ...
  const onSubmit = async (data: TemplateFormValues) => {
    ...
    try {
      ...
      await api.patch(`/campaigns/templates/${templateId}/`, {
      ...
      toast.error(axiosError.response?.data?.detail || 'Failed to update template');
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <div className="flex items-center justify-between">
        ...
        {templateData && (
          <div className="flex items-center gap-2">
            ...
            <Badge variant="secondary">{templateData.usage_count} uses</Badge>
          </div>
        )}
      </div>
